...
const requestNewSlice = createSlice({
	...
	reducers: {
		saveRequest: (state, action) => {
			...
			console.log(
				"tgl kalkual stringify: " + JSON.stringify(action.payload).TglKalkual
			);
		},
		...
		calcEndDate: (state, action) => {
			...
			state.TglKalkual = action.payload.TglKalkual;
			state.Periode = action.payload.Periode;
			state.EDKalkual = dateFns.addMonths(
				parseISO(action.payload.TglKalkual),
				action.payload.Periode
			);
		},
	},
});
